...
v1.2 · PARTIAL IMPLEMENTATION · I01–I04 일부 DB 구조/함수 검증; 파일 import·전체 귀속/통화 인수 검증 미완료

현재 실행 상태는 [11번 문서](11-execution-status.md)를 먼저 본다. 아래 표는 설계 인수 계약이며 실행 결과가 아니다. 직접 게시에 대한 사용자 상시 지시가 있으며, 오래된 승인 테스트는 권한 범위·artifact 무결성 검사로 개정할 대상이다. 매 게시마다 승인을 다시 받는 절차를 새로 넣지 않는다.
...
I01–I04 전체 완료라는 이전 표현을 정정한다. schema/원장 함수와 7개 smoke 검사가 존재하지만, 파일 import 및 contract validator·복수 귀속/비용·혼합 통화·동시성·구버전 재수입은 이 검사로 입증되지 않는다. I05의 실제 보고서 parser와 I06–I09의 direct publisher·공개 수집기·호스팅 적합성·자연 전환은 미완료다. 두 공개 페이지 수정은 선택적 CLI 배포이며 자동 게시기 완성을 의미하지 않는다.

### 실제 실행되는 smoke 검사와 설계 테스트의 구별

`tools/verify_affiliate_core.py`의 옛 F01–F07 표기는 설계 fixture와 다른 사례여서 CORE01–CORE07로 바꿨다. 과거 실행 로그는 보존하고 아래 의미로만 해석한다.

| 실제 ID | 실제 검사 | 입증하지 못하는 것 |
|---|---|---|
| CORE01 | pending journal 1행 | 공급자 파일 전체 검증 |
| CORE02 | pending→confirmed 누계 delta | 동시 posting |
| CORE03 | 동일 v2 재호출 멱등성 | 중첩 파일/다른 parser 전체 멱등성 |
| CORE04 | confirmed 10000→8000 취소 delta | 모든 늦은 정산 유형 |
| CORE05 | 단일 USD 현금 7500 조회 | 복수 비용·귀속 합계 및 순현금 전체 사례 |
| CORE06 | series와 다른 통화 전기 거절 | 모든 조회의 통화 분리 |
| CORE07 | outbox 1행·checkpoint 값 저장 | 실제 export/ack/재전송 및 collector 실행 |

따라서 다음 T01–T22와 설계 F01–F12는 각각 별도 실행 증거가 붙기 전까지 미검증이다.
...
| G1 정산 | 부분 완료 | CORE01–CORE07 smoke 및 schema 회귀; 실제 형식 mapping과 T01–T05/T13–T16 필요 |
| G2 공개 파일럿 | 수동 선택 배포 있음, 전체 미통과 | 호스팅 적합성, T06–T10/T12/T17–T22, 복구·알림·측정 수신 검증 필요 |
