#!/usr/bin/env python3
"""Transactional smoke tests for affiliate migration 013."""
import subprocess
import sys
...
SELECT CASE WHEN (SELECT count(*) FROM affiliate.journal_entries WHERE series_id=:series_id)=1 THEN 'CORE01 pending journal ok' ELSE 'FAIL CORE01' END;
...
SELECT CASE WHEN (SELECT coalesce(sum(delta),0) FROM affiliate.journal_entries WHERE series_id=:series_id AND bucket='pending')=0 AND (SELECT coalesce(sum(delta),0) FROM affiliate.journal_entries WHERE series_id=:series_id AND bucket='confirmed')=10000 THEN 'CORE02 delta posting ok' ELSE 'FAIL CORE02' END;
SELECT CASE WHEN affiliate.post_fact_version(:batch_id,:series_id,'v2','cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc','2026-01-01','reported',0,10000,'USD','row-2','{}')=(SELECT current_version_id FROM affiliate.fact_series WHERE id=:series_id) THEN 'CORE03 idempotent replay ok' ELSE 'FAIL CORE03' END;
...
SELECT CASE WHEN (SELECT coalesce(sum(delta),0) FROM affiliate.journal_entries WHERE series_id=:series_id AND bucket='confirmed')=8000 THEN 'CORE04 reversal delta ok' ELSE 'FAIL CORE04' END;
...
SELECT CASE WHEN (SELECT cash_amount FROM affiliate.v_cash_flow WHERE account_id=:account_id AND currency='USD')=7500 THEN 'CORE05 cash view ok' ELSE 'FAIL CORE05' END;
DO $$ BEGIN
  BEGIN
  ...
  END;
END $$;
SELECT 'CORE06 currency guard ok';
...
SELECT CASE WHEN (SELECT count(*) FROM affiliate.outbox WHERE event_key='fixture-event')=1 AND (SELECT cursor FROM affiliate.pull_checkpoints WHERE consumer_key='fixture-consumer')='cursor-1' THEN 'CORE07 delivery controls ok' ELSE 'FAIL CORE07' END;
ROLLBACK;
"""
...
expected = ["CORE01 pending journal ok", "CORE02 delta posting ok", "CORE03 idempotent replay ok", "CORE04 reversal delta ok", "CORE05 cash view ok", "CORE06 currency guard ok", "CORE07 delivery controls ok"]
...
if missing:
    ...
    sys.exit(1)
print("affiliate core smoke: PASS (7 checks; not full design acceptance)")
